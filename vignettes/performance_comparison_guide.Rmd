...
5%. Harvey's log-linear formulation is conservative under homoskedasticity.
...
studentised variant sacrifices some power for robustness. Harvey's log-linear
regression on the fitted values performs strongly when the variance tracks the
conditional mean, which occurs here because `x1` dominates the signal.

## Practical guidance

* Prefer Breusch–Pagan or Park when theory suggests variance scales with known
  regressors; Harvey is helpful when variance is tied to the fitted mean.
