...
The unweighted variance climbs steeply across the fitted range, which is the
heteroscedasticity the tests detected.

The flat weighted row is **not** evidence that the remedy worked. `fitWLS()`
sets `w_i = 1 / e_i^2`, so each weighted squared residual is 1 by construction
and the row would be flat for any data whatsoever. Read it as a check that the
weighting was applied, not as validation. Weights that mean something have to
come from a variance model estimated in its own right -- the feasible-WLS recipe
in tutorial 2 regresses `log(e^2)` on the fitted values, and for a count response
like this one a quasi-Poisson GLM is the more defensible remedy.
