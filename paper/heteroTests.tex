...
\paragraph{Heteroscedasticity-Consistent Covariance Test}\label{sec:hc-cov}
This diagnostic contrasts the ordinary least squares covariance estimate with the heteroscedasticity-consistent (HC) sandwich estimator $\widehat{\mathrm{Var}}_{HC}(\hat\beta)=(X^\top X)^{-1}X^\top\hat\Omega X(X^\top X)^{-1}$, where $\hat\Omega=\mathrm{diag}(\omega_i e_i^2)$. The weights $\omega_i$ select among the HC0--HC4 variants, which differ in their finite-sample leverage corrections \citep{mackinnon1985some,white1980heteroskedasticity}. A large discrepancy between the two covariance estimates signals heteroscedasticity that would invalidate classical OLS inference. The \texttt{performHCCovarianceTest()} function exposes the full HC0--HC4 family through its \texttt{type} argument.
