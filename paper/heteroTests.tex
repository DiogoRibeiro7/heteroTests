...
The package imports base R and \pkg{MASS}, and uses \pkg{car} and \pkg{sandwich} when they are installed. Timings scale linearly to \(n=50\,000\), where all but one benchmarked test run in under 500\,ms, and simulated power exceeds 80\% against linear, step‐function and autoregressive variance patterns.
...
The argument conventions and the \texttt{htest} return structure are common to all of them, which is what allows the batch runner to treat them interchangeably. Three further components are described in Sections~\ref{sec:design} and~\ref{sec:discussion}: a batch runner with result caching (\texttt{runHeteroTests()}), streaming implementations of the auxiliary‐regression tests for data that do not fit in memory, and a recommendation layer that interprets the diagnostics and proposes corrections such as weighted least squares.
...
\pkg{heteroTests} covers more of these tests than any single alternative in Table~\ref{tab:comparison}, with one hard dependency outside base R. That is a statement about breadth, not about the quality of any individual implementation:
...
The two blocks above run the same four diagnostics: 28 input lines against 10, a reduction of roughly 60\%. Most of the difference is the bridging code that aligns four return formats, which the shared \texttt{htest} structure removes. The saving grows with the number of tests and vanishes for one.
...
Because every test returns the same object, \pkg{heteroTests} supplies \texttt{tidy()}, \texttt{glance()} and \texttt{augment()} methods that apply to all of them, and the returned object records the residual type, trimming, lag order and grouping that produced the statistic. Reproducing a reported result therefore requires the object rather than the call that made it. \pkg{lmtest} and \pkg{car} return \texttt{htest} objects that \pkg{broom} can tidy, but the fields differ between functions, so a caller combining several must map them individually.
...
% TODO[CITATION]: verify or add source for the post-model-selection inference
% claim above (the pre-testing literature, e.g. Leeb and Poetscher on model
% selection and inference). No key was added because none exists in
% RJreferences.bib and the source was not checked.
