...
`man/` is **mixed**: roxygen2 generated 78 of the 130 pages and the other 52
were written by hand.

The hand-written pages are not at risk of being overwritten. roxygen2 skips any
`.Rd` that lacks its `Generated by roxygen2` header, printing `Skipping <file>:
it already exists and was not generated by roxygen2`. That applies whether or
not the function still carries roxygen comments -- 47 of the 52 do, and all 52
survive a `roxygenise()` run byte-identical.

Running `roxygen2::roxygenise()` is still disruptive, for a different reason: it
re-splits topics that the hand-maintained pages deliberately group together. On
the current tree it writes 14 new files and deletes one (`ht_logging.Rd`, which
documents the whole `ht_*` logging family under a single topic). Do not run it
