...
md("## 1. The data and the fitted model",
   "",
   "We model how many seismic stations reported each event (`stations`) for the",
   "1,000 earthquakes near Fiji in R's built-in `quakes` data, as a function of",
   "magnitude (`mag`) and depth (`depth`). The response is a count, and a count's",
   "variance rises with its mean --- the textbook mechanism for error variance",
   "that grows with the fitted value."),

code("data(quakes)",
     "model <- lm(stations ~ mag + depth, data = quakes)",
     "summary(model)"),

md("Both predictors are highly significant *as reported by OLS*. Hold that",
...
md("## 2. What heteroscedasticity costs you",
   ...
   "how badly the classical standard errors are biased here. `sandwich::vcovHC()`",
   "computes the robust matrix directly; below we build it by hand and tabulate",
...
code("# Visualise the same story: 95% intervals, classical vs robust",
     ...
     "       subtitle = \"Robust (HC3) intervals are wider for mag\")"),

md("The robust standard error for `mag` is roughly **1.34&times;** the OLS value,",
   "and the intercept's is **1.30&times;**. The OLS covariance estimator",
   "understates the true sampling variability by about a third for the",
   "coefficients that carry the model --- a",
...
code("white   <- performWhiteTest(model, quakes)",
     "bp      <- performBPTest(model, quakes)",
     "koenker <- performKoenkerTest(model, quakes)",
     "white"),
...
md("## 6. Takeaways",
   ...
   "- The **robust standard error for `mag` was ~1.34&times; the OLS one**: ignoring",
