...
md("# Remediating Heteroscedasticity",
   ...
   "---",
   "",
   "Notebook 1 established that the Boston housing model is heteroscedastic and that",
...
code("library(heteroTests)",
     ...
     "signif(performKoenkerTest(model, boston_housing)$p.value, 3)"),
...
code("v_model <- lm(log(residuals(model)^2) ~ fitted(model))",
     ...
     "wls_bh  <- lm(medv ~ lstat + rm + crim + dis, data = boston_housing, weights = w_hat)",
     "",
     ...
     "  WLS_weighted_resid_var = round(tapply((residuals(wls_bh) * sqrt(w_hat))^2,",
     "                                        cut(fitted(wls_bh), 3), mean), 2))"),
...
code("generateHeteroRecommendations(model, boston_housing)"),
