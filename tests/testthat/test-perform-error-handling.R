...
test_that("core perform*Test functions reject invalid model inputs", {
  ...
  cases <- list(
    ...
    list(name = "rice", fun = performRiceTest, args = list(), expected = c(msg_lm_only, msg_glm, msg_glm_plain)),
    ...
    list(name = "curry_walsh", fun = performCurryWalshTest, args = list(coords = coords), expected = c(msg_lm_only, msg_glm, msg_glm_plain)),
